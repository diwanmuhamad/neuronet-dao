...
module {
  ...
  public class Items(verification : Verification.Verification) {
    ...
    public func getFeaturedItems(itemType : Text, limit : Nat) : [Item] {
      ...
      let sortedItems = Array.sort<Item>(
        typeItems,
        func(a : Item, b : Item) : { #less; #equal; #greater } {
      ...
      let updatedItems = Array.map<Item, Item>(
        ...
        func(item : Item) : Item {
          if (item.id == itemId) {
            ...
            let alreadyLicensed = Array.find<Principal>(
              item.licensedWallets,
              func(wallet : Principal) : Bool {
                wallet == buyer;
              },
            );

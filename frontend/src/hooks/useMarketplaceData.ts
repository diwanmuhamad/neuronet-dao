...
export function useMarketplaceData() {
  ...
  const convertToFeaturedItem = (item: MarketplaceItem, index: number): FeaturedItem => {
    ...
    return {
      title: item.title,
      price: `$${((Number(item.price) / 100_000_000) * 10).toFixed(2)}`,
  ...
  const convertToTrendingItem = (item: MarketplaceItem, index: number): TrendingItem => {
    return {
      rank: index + 1,
      title: item.title,
      category: item.category,
  ...
  const splitIntoColumns = (items: TrendingItem[], columnsCount: number = 4): TrendingItem[][] => {
  ...
  const fetchFeaturedItems = useCallback(async () => {
    ...
    try {
      ...
      setFeaturedPrompts((prompts as MarketplaceItem[]).map(convertToFeaturedItem));
      setFeaturedDatasets((datasets as MarketplaceItem[]).map(convertToFeaturedItem));
      setFeaturedAIOutputs((aiOutputs as MarketplaceItem[]).map(convertToFeaturedItem));
    } catch (err) {
  ...
  const fetchTrendingItems = useCallback(async () => {
    ...
    try {
      ...
      setTrendingPrompts((prompts as MarketplaceItem[]).map(convertToTrendingItem));
      setTrendingDatasets((datasets as MarketplaceItem[]).map(convertToTrendingItem));
      setTrendingAIOutputs((aiOutputs as MarketplaceItem[]).map(convertToTrendingItem));
    } catch (err) {
